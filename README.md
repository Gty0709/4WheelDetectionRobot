...
#### 模块 C 导航演示（图 6 运行 → 图 5 落盘）

建图与检测完成后，用 `map_latest` 中的静态地图与 `waypoints.yaml` 启动 Nav2 航点巡逻。**图 6** 为三终端运行时的 RViz + Gazebo 同屏；任务正常结束后，`waypoint_patrol` 将结果写入 `result/path_<时间戳>/`，并自动生成 **图 5** 的 `mission_trajectory.png`（示例：`path_20260615_131011`，实验 G 参数 **20/20**）。

### 图 6 — AMCL 定位 + Nav2 航点巡逻（运行过程）

`launch_navigation_terminals.py` 依次拉起仿真、AMCL+Nav2、RViz+巡逻节点后的典型画面：

- **左 · `navigation.rviz`**：静态地图 + costmap 膨胀层（紫/青）；全局/局部规划路径（绿线）；**绿球**已驻留、**红球**待访问的检测航点。
- **右 · Gazebo**：Nav2 驱动 MickRobot 实车运动；蓝色扇形为 `/scan`；彩色柱体为场景障碍。

输入数据：`map_20260614_185137`（`slam_map.yaml`、`initial_pose.yaml`、`waypoints.yaml`）。

![图6：AMCL定位与Nav2航点巡逻 RViz+Gazebo](docs/screenshots/fig6-navigation-gazebo.png)

### 图 5 — 航点巡逻任务轨迹（图 6 运行结束后的落盘结果）

图 6 任务完成后自动导出。除轨迹图外，同目录还有 `trajectory.yaml`、`tsp_order.yaml`、`mission_summary.yaml` 等。下图来自 `path_20260615_131011`（实验 G：快速恢复 BT + DWB 小步调参，`dwell=0.75 s`）：

| 图例 | 含义 |
|------|------|
| **浅蓝折线** | 里程计积分得到的实际行驶轨迹 |
| **绿圆** | 已驻留航点（本任务 **20/20** 全部到达，时长约 **1018 s**） |
| **蓝圈 / 橙箭头** | 任务起点与终止位姿 |
| **灰色栅格** | 建图阶段保存的静态地图 |

![图5：航点巡逻任务轨迹 path_20260615_131011](docs/screenshots/fig5-mission-trajectory.png)
...
**前提**：建图 + 检测已完成，`map_latest` 含 `slam_map.yaml`、`waypoints.yaml`、`initial_pose.yaml`（只读）。运行效果见 [图 6](#图-6--amcl-定位--nav2-航点巡逻运行过程)，落盘轨迹见 [图 5](#图-5--航点巡逻任务轨迹图-6-运行结束后的落盘结果)。

**需要模块 A 仿真**：Gazebo 按 `initial_pose.yaml` **prior spawn**（与建图终止位姿一致）；AMCL 在此基础上重定位，Nav2 读取静态地图与检测航点执行 TSP 巡逻。

| 终端 | 作用 | Launch |
|------|------|--------|
| **1** | Gazebo 仿真 | `bringup_classic.launch.py` |
| **2** | AMCL 重定位 + Nav2 | `navigation_humble.launch.py` |
| **3** | RViz + 航点巡逻 | `patrol_rviz.launch.py` |

任务结果写入 `src/navigation_pkg/result/path_<时间戳>/`（`path_latest` 指向最近一次；**不上传 GitHub**）。

#### 方式一：一键启动 3 终端

每隔 **8s** 依次打开上表三个窗口，自动 `source env_humble.bash`（终端 3 内巡逻节点另延迟 25s 启动，以等待 Nav2 lifecycle）：
...
# 调整间隔；无图形终端时用 tmux
python3 scripts/launch_navigation_terminals.py --delay 10
python3 scripts/launch_navigation_terminals.py --tmux
```

指定终端模拟器：`TERMINAL=gnome-terminal python3 scripts/launch_navigation_terminals.py`

**流程**：AMCL 用 `initial_pose.yaml` 重定位 → Held-Karp 曼哈顿 TSP → 逐点 Nav2 导航并驻留 **0.75 s**；RViz **红球**未访问、**绿球**已驻留。

**终端 2 成功标志**：日志须出现**两次** `Managed nodes are active`（localization + navigation），约启动后 **25–30 s**。

#### 方式二：分终端启动

默认会话目录 `SESSION=src/perception_pkg/maps/map_latest`；换成其他建图结果时改三处 `session_dir` / `mapping_prior_file` 即可。

**启动前**（全量重启）：`bash scripts/kill_sim.sh`（仅重启 Nav2 时终端 2 launch 会自动执行 `kill_nav.sh`）。

**终端 1** — Gazebo 仿真（须**最先**启动）：
...
ros2 launch mickrobot_description bringup_classic.launch.py \
  mapping_prior_file:=${SESSION}/initial_pose.yaml
# SSH / headless：gui:=false
```

**终端 2** — AMCL + Nav2（等 Gazebo spawn 完成后再启，约 6 s；启动后约 **25–30 s** 应出现**两行** `Managed nodes are active`）：
...
ros2 launch navigation_pkg navigation_humble.launch.py \
  start_patrol:=false rviz:=false session_dir:=${SESSION}
```

**终端 3** — RViz + 航点巡逻（等终端 2 **导航** lifecycle 也 active 后再启；`patrol_rviz` 内巡逻节点已延迟 25 s）：

```bash
source env_humble.bash
SESSION=src/perception_pkg/maps/map_latest

ros2 launch navigation_pkg patrol_rviz.launch.py session_dir:=${SESSION}
```

**结束任务（Ctrl+C 顺序建议）：**

1. **终端 3** 巡逻 — Ctrl+C（写入 `result/path_<时间戳>/` 与 `mission_trajectory.png`）
2. **终端 2** Nav2 — Ctrl+C
3. **终端 1** Gazebo — Ctrl+C
...
详见 [`src/navigation_pkg/开发文档_C.md`](src/navigation_pkg/开发文档_C.md)、[`docs/NAVIGATION_TUNING.md`](docs/NAVIGATION_TUNING.md) 与 [`src/navigation_pkg/result/README.md`](src/navigation_pkg/result/README.md)。
...
- **[docs/NAVIGATION_TUNING.md](docs/NAVIGATION_TUNING.md)** — Nav2 / DWB 调参记录与任务结果对照
...
| 现象 | 处理 |
|------|------|
| RViz 机器人白模 / 无 `map` TF | 僵尸 `map_server`：`bash scripts/kill_sim.sh` 或 `kill_nav.sh`；终端 2 应有 `map_server: Configuring`→`Activating` |
| 无 TSP / `Still waiting for Nav2` | 终端 2 须**两次** `Managed nodes are active`；仅 AMCL 成功不够。等 **25–30 s** 再开终端 3 |
| `planner_server` / lifecycle 超时 | 已修：Nav2 节点逐个启动 + lifecycle 延后；`kill_nav.sh` **不再**在 Gazebo 运行时清 DDS 共享内存 |
| 机器人不动 | Nav2→`/cmd_vel_nav`→`cmd_vel_guard`→`/cmd_vel`；重启终端 1+2 |
