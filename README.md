...
建图与检测完成后，用 `map_latest` 中的静态地图与 `waypoints.yaml` 启动 Nav2 航点巡逻。**图 6** 为三终端运行时的 RViz + Gazebo 同屏；任务正常结束后，`waypoint_patrol` 将结果写入 `result/path_<时间戳>/`，并自动生成 **图 5** 的 `mission_trajectory.png`（示例：`path_20260615_001514`）。
...
图 6 任务完成后自动导出。除轨迹图外，同目录还有 `trajectory.yaml`、`tsp_order.yaml`、`mission_summary.yaml` 等。下图来自 `path_20260615_001514`：

| 图例 | 含义 |
|------|------|
| **浅蓝折线** | 里程计积分得到的实际行驶轨迹 |
| **绿圆** | 已驻留航点（本任务 20/20 全部到达） |
| **蓝圈 / 橙箭头** | 任务起点与终止位姿 |
| **灰色栅格** | 建图阶段保存的静态地图 |
...
每隔 6s 依次打开上表三个窗口，自动 `source env_humble.bash`：
...
# 调整间隔；无图形终端时用 tmux
python3 scripts/launch_navigation_terminals.py --delay 5
...
**流程**：AMCL 用 `initial_pose.yaml` 重定位 → 驶向最近航点 → 曼哈顿距离矩阵 + Held-Karp TSP → 逐点 Nav2 导航并驻留 2 s；RViz **红球**未访问、**绿球**已驻留。
...
**终端 2** — AMCL + Nav2（等 Gazebo spawn 完成后再启，约 5–10 s）：
...
**终端 3** — RViz + 航点巡逻（等 Nav2 lifecycle 就绪后再启，约 10–15 s）：
